...
uniform float dielectricRoughness;
...
float sampleDielectric(inout vec3 X, inout vec3 D, vec3 N, float ior, inout vec4 rnd)
{
	...
	vec3 mLocal = microfacetSample(rnd, dielectricRoughness);
	...
	if (R >= rand(rnd))
	{
		...
		float G = smithG2(DL, DrL, mLocal, dielectricRoughness);
	...
	else 
	{
		...
		float G = smithG2(DL, DtL, mLocal, dielectricRoughness);
...
bool trace(in vec3 X, vec3 D, inout int numSteps, inout float t)
{
