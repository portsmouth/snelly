...
void main()
{
	...
	vec3 pos = EmitterPos + vec3(1.0, 1.0, 5.0)*(-vec3(0.5) + vec3(rand(state), rand(state), rand(state)));
	vec3 dir = normalize(EmitterDir + 0.0*vec3(rand(state), rand(state), rand(state)));
